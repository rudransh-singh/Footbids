	...
	app.post('/loadCsv', (req, res) => {
		PlayerModel.find().then((doc) => {
			if (doc.length === 0) {
				...
				fs
					.createReadStream(dir + '/public/csv/playerfootbids.csv')
					.pipe(
						parse({
							delimiter: ','
						})
				...
				res.json({ done: false, message: doc.length });
			}
		});
	});
	...
	app.post('/bidDone', (req, res) => {
		TeamModel.findOne({ _id: req.body.tid }).then((team) => {
			PlayerModel.findOne({ _id: req.body.pid }).then((player) => {
				...
				if(team.money < req.body.money) {
					console.log('money less than price');
					res.json({success : false, message : "Money less than price"});
					console.log(res);
					return;
				}
